...
function get_deal($id){
    $name = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."deal where id =".$id);
    return $name;
}

function get_user($id){
    $name = $GLOBALS['db']->getOne("select user_name from ".DB_PREFIX."user where id =".$id);
    return $name;
}

//得到推广人数
function get_popular_count($id){
     $count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."referrals where user_id =".$id);
     return $count;
}
//得到订单状态
function get_off_status($status){
   if($status==1){
     $status ="尚未付款";
   }elseif($status==2){
     $status ="支付待审核";
...
function syn_deal($deal_id)
{
	...
	if($deal_info)
	{
		...
		$deal_info['support_count'] = intval($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_order where deal_id = ".$deal_info['id']." and order_status=3 and is_refund=0"));
		...
		$deal_info['support_amount'] = floatval($GLOBALS['db']->getOne("select sum(deal_price) from ".DB_PREFIX."deal_order where deal_id = ".$deal_info['id']." and order_status=3 and is_refund=0"));
...
function pay_order($order_id)
{
  	...
  	if($GLOBALS['db']->affected_rows()>0) //订单已成功支付
	{
		...
		$price = $order_info['deal_price'];
		$user_parent_id = $user['pid'];
        
        if($user_parent_id!=0){
        	$profit_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."profit where profit_status=1 and deal_id=".$deal_id);
	  
		 if(count($profit_info)>0){
            
		   $fixed_gain = intval($profit_info['fixed_gain']);
		   $rate_gain = floatval($profit_info['rate_gain']);
		   $profit_money = $price*$rate_gain+$fixed_gain;
           $user_id = $GLOBALS['user_info']['id'];
		
		   $profit_user['user_id']=$user_parent_id;
		   $profit_user['amount']=$profit_money;
		   $profit_user['deal_id'] = $deal_id;
		   $profit_user['status'] = 0;
		   $profit_user['create_time'] = get_gmtime();
		   $GLOBALS['db']->autoExecute(DB_PREFIX."profit_user",$profit_user,"INSERT","","SILENT");      
		  }
		...
		if($GLOBALS['db']->affected_rows()>0)
		{
			...
			$deal_type = $GLOBALS['db']->getOne("select type from ".DB_PREFIX."deal where id = ".$order_info['deal_id']);
			...
			if($GLOBALS['db']->affected_rows()>0||($order_info['type']==1)||($deal_type==3)||($deal_type==4))
			{
				$result['status'] = 3;
				...
				//添加数字权益记录  
				//CC 2015-11-23
				$right["user_id"] = $order_info['user_id'];
				$right["deal_id"] = $order_info['deal_id'];
				$right["item_id"] = $order_info['deal_item_id'];
				if($deal_type == 0){
					$right_number = $GLOBALS['db']->getOne("select right_number from ".DB_PREFIX."deal_item where id = ".$right['item_id']);
					$right["right_amount"] = $right_number; 
				}elseif ($deal_type == 3||$deal_type == 4){
					$price_per_right = $GLOBALS['db']->getOne("select price_per_right from ".DB_PREFIX."deal where id = ".$right['deal_id']);
					$right["right_amount"] = intval($order_info['deal_price']/$price_per_right);
				}
				$right["frozen_right_amount"] = 0;
				$right["create_time"] = NOW_TIME;
				$right["update_time"] = NOW_TIME; 
				$GLOBALS['db']->autoExecute(DB_PREFIX."user_right",$right,"INSERT","","SILENT");
...
function get_investor($is_investor){
 ...
 //领投ajax判断
 function  investor_leader_ajax($user_id,$deal_id,$ajax='',$from='web',$info){
 ...
 //申请领投信息入库
 function investor_save_leader($deal_id,$user_id,$cates,$investor_id,$invest,$introduce,$from='web'){
 	if($investor_id>0){
 		if($invest['status']==2){
 			$invest['status']=0;
 		}
...
function get_payment_list($from="pc"){
	if($from=='wap'){
		...
		$payment_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."payment where is_effect = 1 and online_pay=2 $condition order by sort asc ");
		return $payment_list;
	}elseif($from=='pc'){
		$condition.=" and class_name!='Wwxjspay'";
		$payment_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."payment where is_effect = 1 and online_pay=2 $condition order by sort asc ");
...
//2015-11-28 程超
//获取用户当前已经众筹某项目数字权益数量:包含处于订单状态的
function get_zc_right_number($did,$uid){
	$deal_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal where id = ".$did);
	$buy_right_number = 0;
	if($deal_info['type']==3||$deal_info['type']==4){//企业众筹和币创投
 		$orders = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_order where deal_id = ".$deal_info["id"]." and user_id = ".$uid."");
 		if(count($orders)>0){ 
 			foreach ($orders as $k=>$v){
 				$buy_right_number += $v["deal_price"]/$deal_info["price_per_right"];
			}
 		}  
 	}
 	return $buy_right_number;
}
