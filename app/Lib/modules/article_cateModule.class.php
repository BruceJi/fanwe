...
class article_cateModule extends BaseModule
{
	...
	public function rank(){
		$GLOBALS['tmpl']->assign("page_title","推广排名大赛");
		...
		$GLOBALS['tmpl']->assign("other_cate",$artilce_cate_new);
		 //SELECT * FROM `fanwe_referrals` group by user_id order by count(user_id) desc
		$rank = $GLOBALS['db']->getAll("select  * from ".DB_PREFIX."referrals group by user_id order by count(user_id) desc limit 0,20");
		for ($i=0; $i <count($rank) ; $i++) { 
			$rank[$i]['index'] = $i+1;
		}
		$GLOBALS['tmpl']->assign("rank",$rank);		
		$GLOBALS['tmpl']->display("rank.html");
	}
}
?>