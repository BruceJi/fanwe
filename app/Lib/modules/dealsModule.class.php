	...
	{	
       ...
       if($type ==1){
			...
			//	set_dynamic_cache("INDEX_CITY_LIST",$city_list);
       		...
       		if(!$city_list)
			{
				$city_list = $GLOBALS['db']->getAll("select province from ".DB_PREFIX."deal where type=3 and is_effect =1 group by province  order by sort asc");
			//	set_dynamic_cache("INDEX_CITY_LIST",$city_list);
			}
       }elseif($type == 4){
       		if(!$city_list)
		...
		if(count($cate_ids)>0 && ($type != 3 && $type != 4) ) //企业众筹或者币创投无行业
		{
...
//		}
		...
		$url_list=array(
 			'price_url'=>url("deals",$temp_param_price),
			'focus_url'=>url("deals",$temp_param_focus),
			'time_url'=>url("deals",$temp_param_time),
			'cp_url'=>url("deals",$temp_param_cp),
		);
 		//========
		if($price>0){
			if($price==1){
...
//		var_dump($orderby);
 		...
 		if($type==1){
 			...
 			$GLOBALS['tmpl']->assign("deal_list_enterprise",$result['list']);
		}elseif($type==4){
 			$GLOBALS['tmpl']->assign("deal_list_coin",$result['list']);
		}else{
	...
	// 众筹买房列表
	public function financ_house()
	{
		$GLOBALS['tmpl']->display("deals_financ_house.html");
	}

	// 团购买房列表
	public function group_house()
	{
		// 区域
		$city_list = load_dynamic_cache("INDEX_CITY_LIST"); 
        if(!$city_list)
		...
		$GLOBALS['tmpl']->assign("area_array_str",$area_array_str);
		$GLOBALS['tmpl']->assign("hot_area",array_filter($hot_area));
		$GLOBALS['tmpl']->display("deals_group_house.html");
	}

	// 众筹建房列表
	public function schedule()
	{
		$GLOBALS['tmpl']->display("deals_schedule.html");
	}
}
?>