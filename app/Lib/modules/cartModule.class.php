...
class cartModule extends BaseModule
{
	...
	{		
		...
		$ticket_list =array();
		/*showErr($ticket_user_list[0]['ticket_id']);*/
		if(count($ticket_user_list)>0){
	...
	public function index_(){ 
		...
		$zc_amount = intval($_REQUEST['zc_amount']); 

		$deal_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal where is_delete = 0 and is_effect = 1 and id = ".$did);
		if(!$deal_info)
		{
			app_redirect(url("index"));
		}
		elseif($deal_info['begin_time']>NOW_TIME||($deal_info['end_time']<NOW_TIME&&$deal_info['end_time']!=0))
		{
			app_redirect(url("deal#show",array("id"=>$did)));
		}
		...
		/*$buy_right_number = 0;
 		...
 		if($deal_info["zc_amount_limit"]>0 && $zc_amount+$buy_right_number>$deal_info["zc_amount_limit"]){
 			showErr("对不起，您众筹数字权益份额已经超过上限！",0,url("deal#show",array("id"=>$did)));
		}

		if($zc_amount<=0){
			showErr("对不起，众筹支持数量不得小于0",0,url("deal#show",array("id"=>$did)));
		}
		...
		$zc_cost_money = $zc_amount*$deal_info["price_per_right"]; 
		$GLOBALS['tmpl']->assign("zc_cost_money",$zc_cost_money); 
		$GLOBALS['tmpl']->assign("zc_amount",$zc_amount); 
		...
		$GLOBALS['tmpl']->display("cart_index_.html");
	}
	...
	public function check()
	{
		...
		$zc_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_order where deal_id = ".$deal_info["id"]." and user_id = ".$GLOBALS['user_info']['id']);
		if($deal_info["zc_count_limit"]!=0 and $zc_count >= $deal_info["zc_count_limit"]){ 
			showErr("对不起，您参与次数超过上限，已经无法继续参与本项目！",$ajax,url("deal#show",array("id"=>$deal_info["id"])));
		}
		...
		if($deal_item['is_delivery']==1)
		{
			...
			if($consignee_id==0)//签收人id
			{
				$consignee_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_consignee where user_id = ".intval($GLOBALS['user_info']['id']));
				if($consignee_list)//签收人列表
				{
	...
	public function check_()
	{
		...
		/*$buy_right_number = 0;
 		...
 		if($deal_info["zc_amount_limit"]>0 && $zc_amount+$buy_right_number>$deal_info["zc_amount_limit"]){
 			showErr("对不起，您众筹数字权益份额已经超过上限！",$ajax,url("deal#show",array("id"=>$deal_info["id"])));
		}
		...
		showSuccess("",$ajax,url("cart#pay_",array("id"=>$id,"pay_money"=>$pay_money)));  
	}
	...
	public function go_pay()
	{
        ...
        if(count($ticket)>0){
           ...
           if($ticket_type==0){
                $less_money = $ticket['money'];
                $deal_item['total_price'] = $deal_item['total_price']-$less_money;
                $GLOBALS['db']->query("update ".DB_PREFIX."user_ticket set tickets_status = 0 where ticket_id =".$ticket_id);
       ...
       // $deal_item['total_price_format'] = number_price_format($deal_item['total_price']);
		...
		$order_info['total_price'] = $deal_item['total_price'];
