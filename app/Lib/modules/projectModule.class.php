...
class projectModule extends BaseModule
{
	...
	public function index_enterprise()
	{
		$GLOBALS['tmpl']->display("project_index_enterprise.html");
	}

	public function index_coin()
	{
		$GLOBALS['tmpl']->display("project_index_coin.html");
	}
	...
	{	
		...
		$GLOBALS['tmpl']->display("project_add_enterprise.html");
	}
	...
	{	
		...
		$GLOBALS['tmpl']->display("project_add_coin.html");
	}
	...
	public function submit_deal_enterprise(){ 
		$ajax = intval($_REQUEST['ajax']);	
		if(!check_ipop_limit(get_client_ip(),"project_submit_deal_enterprise",5))
		showErr("提交太频繁",$ajax,"");	
	...
	public function save_deal_item()
	{
		...
		else
		{
			...
			$GLOBALS['db']->query("update ".DB_PREFIX."deal set deal_extra_cache = '' where id = ".$data['deal_id']);
			showSuccess("保存成功",$ajax,url("project#add_item",array('id'=>$id)));
		}
