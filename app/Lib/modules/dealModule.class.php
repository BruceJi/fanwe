 ...
 class dealModule extends BaseModule
{
	...
	public function init_deal_page($deal_info)
	{
		...
		$type_2=0;
		$type_array=array();
		$deal_info["total_virtual_right"] = 0;//到目前为止累积权益份额
		$deal_info["item_right_amount"] = 0;//
		...
		foreach ($deal_info['deal_item_list'] as $k=>$v){
			...
			$deal_info['deal_item_list'][$k]['virtual_person_list']=intval($v['virtual_person']+$v['support_count']);
 			...
 			if($deal_info['type']==0){//产品众筹 
	 			if($v['type']==0){//计算产品众筹+产品回报累积份额 
	 				$support_num = floatval( $deal_info['deal_item_list'][$k]['virtual_person_list'] );
	 				$right_num = floatval($v['right_number']);
	 				$mul = $support_num * $right_num ;  
	 				$deal_info["total_virtual_right"] += $mul;  
		 		}  

		 		//限制人数限制，默认要求限制
		 		if($v["type"]==2){//概率型回报
		 			$deal_info["item_right_amount"] += $v["repaid_limit_user"] * $v["right_number"];
		 		}else if($v["type"]==0){//产品回报
		 			$deal_info["item_right_amount"] += $v["limit_user"] * $v["right_number"];
		 		}//无私奉献不产生数字权益
	 		}
 		}
 		if($deal_info['type']==3||$deal_info['type']==4){
 			$deal_info["total_virtual_right"] = $deal_info['support_amount']/$deal_info['price_per_right'];
 		}
		...
		foreach ($deal_item_list as $k=>$v){
			if($v['limit_user']>0){
				...
				$deal_item_list[$k]['right_number'] = intval($deal_item_list[$k]['right_number']);
...
//		$GLOBALS['tmpl']->assign('comment_count',$comment_count);
	...
	public function addProfit(){
		...
		$deal_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal where id = ".$id." and is_delete = 0 and (is_effect = 1 or (is_effect = 0 and user_id = ".intval($GLOBALS['user_info']['id'])."))");
        $GLOBALS['tmpl']->assign("deal_info",$deal_info);
        $GLOBALS['tmpl']->display("deal_addProfit.html");
	}
	//增加收益活动
    public function profitSave(){
         //验证   后期的验证通过AJAX
         $deal_id = intval($_REQUEST['id']);
         $profit_info['deal_id'] = intval($_REQUEST['id']);
         $profit_info['create_time'] = NOW_TIME;
         $profit_info['fixed_gain'] = intval(trim($_REQUEST['fixed_gain']));
         $profit_info['rate_gain'] = floatval(trim($_REQUEST['rate_gain']));
         
         $deal_profit = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."profit where profit_status=1 and deal_id=".$deal_id);
         if(count($deal_profit)>0){
              showErr("该众筹活动已经有正在进行的收益活动");
         }else{
         	 $is_save = $GLOBALS['db']->autoExecute(DB_PREFIX."profit",$profit_info,"INSERT","","SILENT");
           if($is_save){
               showSuccess("添加成功");
             }else{
         	 showErr("添加失败");
    ...
    public function addTicket(){
       if(!$GLOBALS['user_info'])
		...
		$deal_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal where id = ".$id." and is_delete = 0 and (is_effect = 1 or (is_effect = 0 and user_id = ".intval($GLOBALS['user_info']['id'])."))");
        $GLOBALS['tmpl']->assign("deal_info",$deal_info);
        $GLOBALS['tmpl']->display("deal_addTicket.html");
    }
   
    public function profitTicketSave(){
        //判断账户余额是否可以方法代金券
        $id = $GLOBALS['user_info']['id'];
        $user_money = $GLOBALS['db']->getOne("select money from ".DB_PREFIX."user where id =".$id);
        $quantity = intval($_REQUEST['quantity']);
        $money = intval($_REQUEST['money']);
        $money_amount = $money*$quantity;
        if($user_money<$money_amount){
            showErr("您的账户余额不足");
         }else{
            //验证：后期通过Ajax验证
            $profit_ticket['ticket_type'] =intval($_REQUEST['ticket_type']);
            $profit_ticket['money'] =$money;
            $profit_ticket['quantity'] =$quantity;
            $profit_ticket['rest_quantity'] = $quantity;
            $profit_ticket['type'] =intval($_REQUEST['type']);
            $profit_ticket['value'] =$_REQUEST['value'];
            $profit_ticket['deal_id'] =intval($_REQUEST['id']);
            $profit_ticket['founder_id'] =$id; 
            $profit_ticket['use_time'] = intval($_REQUEST['use_time']);
            $profit_ticket['create_time'] = get_gmtime();
            
            //扣除账户余额
            $user['money'] = $user_money-$money_amount;

            $is_update = $GLOBALS['db']->autoExecute(DB_PREFIX."user",$user,"UPDATE","id=".$id);
            $is_save = $GLOBALS['db']->autoExecute(DB_PREFIX."ticket",$profit_ticket,"INSERT","","SILENT");
  ...
  public function getTicket(){
  	     if(!$GLOBALS['user_info'])
		 {
			showErr("请先登录账号",$ajax,url("user#login"));
		 }
         $ticket_id = intval($_REQUEST['id']);
         $ticket_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."ticket where id=".$ticket_id);

         $user_id = $GLOBALS["user_info"]["id"];
         $ticket_user = $GLOBALS['db']->getAll("select *from ".DB_PREFIX."user_ticket where user_id =".$user_id." and ticket_id =".$ticket_id);
         if(count($ticket_user)>0){
            showErr("每个用户领取一个活动的券数是唯一的");
         }
         if($ticket_info['rest_quantity']>0){
            $ticket_info['rest_quantity'] = $ticket_info['rest_quantity'] - 1;  
            $user_id = intval($GLOBALS['user_info']['id']);
            $user_ticket_info['ticket_id'] = $ticket_id;
            $user_ticket_info['user_id'] = $user_id;
            $user_ticket_info['quantity'] = 1;
            $user_ticket_info['tickets_status'] = 1;
            $user_ticket_info['get_time']=get_gmtime();
            $is_update = $GLOBALS['db']->autoExecute(DB_PREFIX."ticket",$ticket_info,"UPDATE","id=".$ticket_id);
            $is_save = $GLOBALS['db']->autoExecute(DB_PREFIX."user_ticket",$user_ticket_info,"INSERT","","SILENT");           
            //插入user_ticket
            if($is_update&&$is_save){
                 showSuccess("领取成功");
            }else{
            	showErr("领取失败");
            }
