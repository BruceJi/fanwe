...
//		}
	...
	*/	
	...
	public function zhg_bind(){ 
		if(!$GLOBALS['user_info'])
		app_redirect(url("user#login")); 
		$GLOBALS['tmpl']->assign("page_title",'众股平台账户绑定');
		
		$zhg_id = $GLOBALS['db']->getOne("select zhg_id from ".DB_PREFIX."user where id = ".intval($GLOBALS['user_info']['id']));
		$zhg_name = $GLOBALS['db']->getOne("select zhg_name from ".DB_PREFIX."user where id = ".intval($GLOBALS['user_info']['id']));
		if($zhg_id){
			$GLOBALS['tmpl']->assign("zhg_id",$zhg_id);
		}
		if($zhg_name){
			$GLOBALS['tmpl']->assign("zhg_name",$zhg_name);
		}

		$GLOBALS['tmpl']->display("settings_zhg_bind.html");
	}
