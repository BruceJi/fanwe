...
class accountModule extends BaseModule
{
	...
	{	
		...
		{
			$condition .= " (de.type=0 or de.type=3 or de.type=4) ";
		}
		elseif (app_conf("INVEST_STATUS")==2)
		{
			$condition .= " (de.type=1 or de.type=3 or de.type=4) ";
		}
		...
		if($deal_ids!=null){
			$deal_list_array=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal where  is_effect = 1 and (type=0 or type=3 or type=4) and is_delete = 0 and id in (".implode(',',$deal_ids).")");
			$deal_list=array();
	...
	public function offline_pay(){
          //$GLOBALS['tmpl']->showErr();     
         
         $now = get_gmtime();
		 $amount = floatval($_REQUEST['amount']);
		 //$bank = $_REQUEST['user_bank'];
		 $user_id = intval($GLOBALS['user_info']['id']);
		 $user_name = $GLOBALS['db']->getOne("select user_name from ".DB_PREFIX."user where id =".$user_id);

         $offline_order['amount'] = $amount;
         $offline_order['user_id'] = $user_id;
         $offline_order['user_name'] = $user_name;
        // $offline_order['user_bank'] = $user_bank;
         $bank = intval($_REQUEST['sys_bank_id']);

         $offline_order['create_time'] =get_gmtime(); 
         $offline_order['status'] =1;
         $offline_order['sys_bank_id'] = $bank ;
         

         $GLOBALS['db']->autoExecute(DB_PREFIX."offpay",$offline_order,"INSERT","","SILENT");

         $order_id = $GLOBALS['db']->getOne("select id from ".DB_PREFIX."offpay where create_time =".$now);
         echo $order_id;

         $off_pay_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."offpay where user_id =".$user_id);
        
         /* $order_id = M("offpay")->add($offline_order);*/
        }

    public function get_sys(){
    	$id = intval($_REQUEST['id']);
    	$sys_bank = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."sys_bank where id =".$id);
    	echo json_encode($sys_bank);
    }

    public function off_record(){
    	if(!$GLOBALS['user_info'])
		{
			app_redirect(url("user#login"));
		}
		...
		$pay_money = $GLOBALS['db']->getOne("select sum(amount) from ".DB_PREFIX."offpay where (status =2 or status =3) and user_id=".$user_id);
		...
		$off_pay_list_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."offpay where user_id =".$user_id);

		
		$off_pay_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."offpay where user_id =".$user_id." order by create_time desc limit ".$limit);

		$page = new Page($off_pay_list_count,$page_size);   //初始化分页对象
		$p  =  $page->show();
		$GLOBALS['tmpl']->assign('pages',$p);

		$GLOBALS['tmpl']->assign('off_pay_list',$off_pay_list);
		$GLOBALS['tmpl']->assign('pages',$p);
		$GLOBALS['tmpl']->display("account_off_record.html");

    }

	public function confirm_offPay(){

	    $order_id = intval($_REQUEST['order_id']);
	    $offline_order['user_bank'] = $_REQUEST['user_bank'];
	    $offline_order['user_account'] = $_REQUEST['user_account'];
	    $offline_order['user_phone'] = $_REQUEST['user_phone'];
	    $offline_order['status'] = 2;
       
	    //$GLOBALS['db']->autoExecute(DB_PREFIX."offpay",$offline_order,"id=".$order_id);
	    $GLOBALS['db']->autoExecute(DB_PREFIX."offpay",$offline_order,"UPDATE","id=".$order_id);

	    $off_pay_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."offpay where user_id =".$user_id);

	}
	
    public function off_pay_list(){
    	$user_id = $GLOBALS['user_info']['id'];
    	$off_pay_list = $GLOBALS['db']->getAll("select  * from ".DB_PREFIX."offpay where user_id=".$user_id." order by create_time desc limit 0,5");
    	for ($i=0; $i <count($off_pay_list) ; $i++) { 
    		$off_pay_list[$i]['create_time'] = to_date($off_pay_list[$i]['create_time']);
    	}
    	//echo count($off_pay_list);
    	echo json_encode($off_pay_list);
    }
    
    public function cancle(){
    	$order_id = intval($_REQUEST['id']); 	
        $offline_order['status'] = 4;
    	$GLOBALS['db']->autoExecute(DB_PREFIX."offpay",$offline_order,"UPDATE","id =".$order_id);
    }

    public function sys_bank(){
    	$sys_bank_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."sys_bank");
    	echo json_encode($sys_bank_list);
    }
	...
	public function project()
	{
		...
		{	
			$condition = " (type=0 or type=3 or type=4) ";	
		}
		elseif (app_conf("INVEST_STATUS")==2)
		{
			showErr("产品众筹已经关闭");
	...
	public function profit_details(){
		...
		$user_profit_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."profit_user where status=1 AND deal_id=".$deal_id);
		$totalMoney = 0;
		for ($i=0; $i <count($user_profit_list); $i++) { 
			 if(is_array($user_profit_list[$i])){ 
                 $totalMoney =$totalMoney+$user_profit_list[$i]['amount'];
			 }
		}

		$GLOBALS['tmpl']->assign("total",$totalMoney);
		$GLOBALS['tmpl']->assign('user_profit_list',$user_profit_list);

		$GLOBALS['tmpl']->display("account_profit_details.html");
	}
    ...
    public function ticket_details(){
        ...
        if(!$GLOBALS['user_info'])
		   app_redirect(url("user#login"));
		$user_id = $GLOBALS['user_info']['id'];
        $ticket_user_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_ticket where tickets_status = 1 AND user_id=".$user_id);
        $ticket_list = array();
        for ($i=0; $i <count($ticket_user_list); $i++) {
             if(is_array($ticket_user_list[$i])){
                 $ticket_id = $ticket_user_list[$i]['ticket_id'];
                 $ticket = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."ticket where id=".$ticket_id);
                 $ticket_get_time = $ticket_user_list['get_time'];
                 $ticket['get_time'] = $ticket_user_list[$i]['get_time'];
                 $ticket['use_end'] = $ticket['get_time']+$ticket['use_time']*24*3600;
                 array_push($ticket_list, $ticket);
             }           
        }
        /*showSuccess(count($tickets));*/
        $GLOBALS['tmpl']->assign("ticket_list",$ticket_list);
        $GLOBALS['tmpl']->display("account_ticket_details.html");
    }

    public function user_profit_details(){
    	$GLOBALS['tmpl']->assign("page_title","推广收益明细");
		if(!$GLOBALS['user_info'])
		   app_redirect(url("user#login"));
    	$user_id = $GLOBALS['user_info']['id'];

    	$total_profit = $GLOBALS['db']->getOne("select sum(amount) from ".DB_PREFIX."profit_user where user_id =".$user_id);
    	$sent_profit = $GLOBALS['db']->getOne("select sum(amount) from ".DB_PREFIX."profit_user where status =1 AND user_id =".$user_id);
    	
    	$GLOBALS['tmpl']->assign("total_profit",$total_profit);
    	$GLOBALS['tmpl']->assign("sent_profit",$sent_profit);
		...
		$profit_user_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."profit_user where user_id =".$user_id." limit ".$limit);
		$profit_user_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."profit_user where user_id = ".$user_id);
    	
		
	
		$page = new Page($profit_user_count,$page_size);   //初始化分页对象 		
		$p  =  $page->show();
		$GLOBALS['tmpl']->assign('pages',$p);
		$GLOBALS['tmpl']->assign("profit_user_list",$profit_user_list);		

		$GLOBALS['tmpl']->display("account_profit.html");
	...
	{	
		$GLOBALS['tmpl']->assign("page_title","数字权益明细");
		if(!$GLOBALS['user_info'])
		app_redirect(url("user#login"));
		...
		if($deal_status>0){
			switch($deal_status){
				case 1://等待交易
					$condition .=" and is_effect=1 and trade_begin_time>".NOW_TIME." ";
					break;
				case 2://交易中
					$condition .=" and is_effect=1 and is_success = 1 and trade_begin_time<".NOW_TIME." and  trade_end_time>".NOW_TIME." ";
					break;
				case 3://交割中
					$condition .=" and is_effect=1 and is_success = 1 and trade_end_time<".NOW_TIME." ";
					break;
			}
			$GLOBALS['tmpl']->assign('deal_status',$deal_status);
		} 

		$right_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_right where user_id = ".intval($GLOBALS['user_info']['id'])." order by create_time desc limit ".$limit);
		$right_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user_right where user_id = ".intval($GLOBALS['user_info']['id']));
		$right_lists = array();
		foreach($right_list as $k=>$v){
			$deal = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal where id = ".$v["deal_id"]." and ".$condition);						
			$deal = $deal[0];
			if( $deal && ($deal["type"]==0 || $deal["type"]==3 ||$deal["type"]==4) ){//针对产品众筹，企业众筹，币创投
				$right_list[$k]["deal_name"] = $deal["name"];
				$right_list[$k]["deal_image"] = $deal["image"];
				$right_list[$k]["price_per_right"] = $deal["price_per_right"];
				if($deal["is_effect"]==1 && $deal["trade_begin_time"]>NOW_TIME){
					$right_list[$k]["deal_status"] = 1;
				} elseif($deal["is_effect"]==1 && $deal["is_success"]==1 && $deal["trade_begin_time"]<NOW_TIME && $deal["trade_end_time"]>NOW_TIME)
				{
					$right_list[$k]["deal_status"] = 2;
				} elseif($deal["is_effect"]==1 && $deal["is_success"]==1 && $deal["trade_end_time"]<NOW_TIME){
					$right_list[$k]["deal_status"] = 3;
				} else{
					$right_list[$k]["deal_status"] = 0;
				}
				$right_lists[] = $right_list[$k];
			}
		} 

		$parameter_str="&".implode("&",$parameter);
		$page = new Page($right_count,$page_size,$parameter_str);   //初始化分页对象 		
		$p  =  $page->show();
		$GLOBALS['tmpl']->assign('pages',$p);
		$GLOBALS['tmpl']->assign('right_list',$right_lists);

		$GLOBALS['tmpl']->display("account_right_list.html"); 
	}
	...
	public function view_order()
	{
		...
		$id = intval($_REQUEST['id']);
		$order_info = $GLOBALS['db']->getRow("select deo.*, di.is_delivery as is_delivery,d.transfer_share as transfer_share,d.limit_price as limit_price from ".DB_PREFIX."deal_order as deo LEFT JOIN (".DB_PREFIX."deal as d,".DB_PREFIX."deal_item as di) on (deo.deal_id = d.id and  di.id = deo.deal_item_id )  where deo.id = ".$id." and deo.user_id = ".intval($GLOBALS['user_info']['id']));
		if(!$order_info)
		{
	...
	public function set_repay(){
		...
		if($_REQUEST['type'] ==1)
			...
			$order_info = $GLOBALS['db']->getRow("select d.*,di.description as item_description , di.is_delivery as is_delivery from ".DB_PREFIX."deal_order as d left join ".DB_PREFIX."deal_item as di on di.id=d.deal_item_id where d.id = ".$order_id." and d.order_status = 3 and d.is_refund = 0 and d.deal_item_id >0 order by d.create_time desc");
	...
	public function submitrefund()
	{
		...
		$money = floatval($_REQUEST['money']);
		$fee1 = floatval($_REQUEST['fee1']);
		$fee2 = floatval($_REQUEST['fee2']);
		...
		$refund_data['real_money'] = $money*(1-$fee2/100) - $fee1;
	...
	public function money_carry()
	{
		...
		$bank_info=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_bank where id=".$user_bank_id." and user_id=".$GLOBALS['user_info']['id']);
 		...
 		$fee = $GLOBALS['db']->getOne("select withdraw_fee from ".DB_PREFIX."sys limit 0,1");
 		$pos = strpos($fee,":");
 		$len = strlen($fee);
 		$fee1 = intval(substr($fee,0,$pos));
 		$fee2 = floatval(substr($fee,$pos+1,$len-$pos-1));
 		$GLOBALS['tmpl']->assign('fee1',$fee1);
 		$GLOBALS['tmpl']->assign('fee2',$fee2*100);//手续费
