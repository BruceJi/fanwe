...
function bind_project_form()
{
    ...
    $("#profit_form").bind("submit",function(){
    	if($(this).find("input[name='fixed_gain']").val()==''||$(this).find("input[name='fixed_gain']").val()<0)
		{
			$.showErr("请填写正确的固定收益");
			return false;
		}
		if($(this).find("input[name='rate_gain']").val()==''||parseFloat($(this).find("input[name='rate_gain']").val())<0||parseFloat($(this).find("input[name='rate_gain']").val())>1)
		{
			$.showErr("请填写正确的比例收益");
			return false;
		}
    ...
    $("#ticket_form").bind("submit",function(){
       if($(this).find("input[name='money']").val()==''||parseFloat($(this).find("input[name='money']").val())<0)
		{
			$.showErr("请填写正确的金额");
			return false;
		}
		if($(this).find("input[name='quantity']").val()==''||parseFloat($(this).find("input[name='quantity']").val())<0)
		{
			$.showErr("请填写正确的发放数量");
			return false;
		}
	...
	$("#save_ticket").bind("click",function(){
		$("#ticket_form").submit();
	});
