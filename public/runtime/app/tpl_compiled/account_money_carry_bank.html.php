...
<style type="text/css">
	...
	.cashdraw_tips .cashdraw_tip_header { background: url("<?php echo $this->_var['TMPL']; ?>/images/reg_tip_header_t.gif") no-repeat scroll 0 17px transparent; float: left; height: 47px; position: relative; width: 6px; z-index: 1000; }
	.cashdraw_tips .cashdraw_tip_body_container { background: url("<?php echo $this->_var['TMPL']; ?>/images/reg_tip_bg_bottom.gif") repeat-x scroll center bottom transparent; float: left; margin-left: -1px; padding-bottom: 2px; position: relative; }
	...
	<div class="homeright pageright f_r">
		<div class="account_money">
			<div class="list_title clearfix">
				<div <?php if ($this->_var['module'] == 'account' && ( $this->_var['action'] == 'money_carry_bank' || $this->_var['action'] == 'money_carry' )): ?>class="cur"<?php endif; ?>>
					<a href="<?php
echo parse_url_tag("u:account#money_carry_bank|"."".""); 
?>">我要提现</a>
				</div>
				<div <?php if ($this->_var['module'] == 'account' && $this->_var['action'] == 'refund'): ?>class="cur"<?php endif; ?>>
					<a href="<?php
...
?>">提现日志</a>
			...
			<div class="list_conment">
				...
				<div class="cf" id="SITE_CARRY">
					...
					<?php $_from = $this->_var['banks']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'bank');if (count($_from)):
    foreach ($_from AS $this->_var['bank']):
?>
					<div class="item_bank">
						<div class="tip" onclick="window.location.href='<?php
echo parse_url_tag("u:account#money_carry|"."id=".$this->_var['bank']['id']."".""); 
?>'">
				         <img src="<?php echo $this->_var['TMPL']; ?>/images/bank/<?php echo $this->_var['bank']['bank_id']; ?>.jpg" alt="" style="*margin-bottom:-4px;_margin-bottom:-6px;">
						 <p><?php echo $this->_var['bank']['real_name']; ?></p>
						 <p>尾号：<?php 
$k = array (
  'name' => 'substr',
  'v' => $this->_var['bank']['bankcard'],
  'c' => '-3',
);
echo $k['name']($k['v'],$k['c']);
?></p>
						...
						<div class="edit_card">
							<a href="javascript:void(0);" class="a_link J_deal_bank" dataid="<?php echo $this->_var['bank']['id']; ?>">删除此银行卡</a>
						</div>
					</div>
...
?>&lt;td height=\'22\'&gt;<?php echo $this->_var['fee']['name']; ?>&lt;/td&gt;<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>&lt;/tr&gt;&lt;tr class=\'bg\'&gt;<?php $_from = $this->_var['fee_config']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'fee');if (count($_from)):
    foreach ($_from AS $this->_var['fee']):
?>&lt;td&gt;<?php echo $this->_var['fee']['fee_format']; ?>&lt;/td&gt;<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>&lt;/tr&gt;&lt;/table&gt;',70,290)"><br>
					            	<?php endif; ?>
					            </div>
					            <div class="blank0"></div>
							</div>
...
<div class="blank"></div>
 <script type="text/javascript" src="<?php echo $this->_var['TMPL']; ?>/js/jquery.bank.js"></script>	
	...
	jQuery(function(){
		...
		<?php if ($this->_var['is_tg'] && $this->_var['user_info']['ips_acct_no']): ?>
			...
			$("input[name='money']").bind("blur",function(){
				if($(this).val()){
					get_pay_url="<?php
echo parse_url_tag("u:ajax#get_carry_fee|"."".""); 
?>";
			...
			$("#Jcarry_submit").bind("click",function(){
				...
				url = APP_ROOT+"/index.php?ctl=collocation&act=DoDwTrade&user_type=0&user_id=<?php echo $this->_var['user_info']['id']; ?>"+"&pTrdAmt="+$("input[name='money']").val();
				location.href=url;
			});
			...
			$(".bind_bank").bind('click',function(){
				window.location.href= APP_ROOT + "/index.php?ctl=collocation&act=BindBankCard&user_id=<?php echo $this->_var['user_info']['id']; ?>";
			});
		...
		$(".J_deal_bank").click(function(){
			...
			$.showConfirm("确定要删除吗",function(){
				$.ajax({
					url:'<?php
echo parse_url_tag("u:account#delbank|"."".""); 
?>',
					data:query,
					type:"post",
					dataType:"json",
		...
		$("#add_bank").click(function(){
			$.ajax({
				url:'<?php
echo parse_url_tag("u:ajax#add_bank|"."".""); 
?>',
				dataType:"json",
				success:function(result){
					if(result.status==1)
