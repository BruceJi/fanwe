...
class DealAction extends CommonAction{
	...
	{	
		$deal_info['support_amount'] = floatval($GLOBALS['db']->getOne("select sum(deal_price) from ".DB_PREFIX."deal_order where deal_id = 64 and order_status=3 and is_refund=0"));
		$now=get_gmtime();
	...
	public function edit() {		
		...
		$vo['trade_begin_time'] = $vo['trade_begin_time']!=0?to_date($vo['trade_begin_time']):'';
		$vo['trade_end_time'] = $vo['trade_end_time']!=0?to_date($vo['trade_end_time']):'';
 		$this->assign ( 'vo', $vo );
		$this->assign ( 'aa', 10 );
		$cate_list = M("DealCate")->findAll();
		$cate_list = D("DealCate")->toNameFormatTree($cate_list);
	...
	public function insert() {
		...
		if(!check_empty($data['zc_count_limit']) or floatval($data['zc_count_limit'])<0)//为空或者<0设置为默认值
		{
		...
		if(floatval($data['digital_right_per_good'])<=0){
			$this->error("单个产品权益份额要大于0");
		}
		...
		if (false !== $list) {
			...
			//添加deal_code 
			$short_name = "EP";
			$code = $short_name."".$list; 
			$zero_num = 8 - strlen($code);
			$deal_code = $short_name;
			for( $i=0; $i<$zero_num; $i++ ){
				$deal_code = $deal_code."0";
			}
			$deal_code = $deal_code."".$list; 
			$data["deal_code"] = $deal_code;
			$GLOBALS['db']->autoExecute(DB_PREFIX."deal",$data,"UPDATE","id=".$list,"SILENT");
	...
	public function insert_enterprise(){
		...
		//开始验证有效性
		$this->assign("jumpUrl",u(MODULE_NAME."/add_enterprise"));
		if(!check_empty($data['name']))
		{
			$this->error("请输入名称");
		}		 
		...
		if(!check_empty($data['zc_amount_limit']) or floatval($data['zc_amount_limit'])<0)//为空或者<0设置为默认值
		{
			$data['zc_amount_limit'] = 0;
		}
		/*if(intval($data['cate_id'])==0)
		{
			$this->error("请选择分类");
		...
		#}
		if(floatval($data['digital_right_amount'])<=0){
			$this->error("数字权益总份额要大于0");
		} 
		if(floatval($data['price_per_right'])<=0){
			$this->error("数字权益单价要大于0");
		}  
	...
	public function insert_deal_item() {
		...
		if(!check_empty($data['right_number'])&&$data['type']==0)
		{
			$this->error("请输入权益数量");
		}  
		if(!check_empty($data['repaid_limit_user'])&&$data['type']==2)
		{
			$this->error("请输入需要回报用户数量");
		}  

		if($data['repaid_limit_user']<=0&&$data['type']==2){
			$this->error("获取回报人数必须大于0");	
		} 
		if($data['repaid_limit_user']>$data['limit_user']&&$data['type']==2){
			$this->error("获取回报人数不能大于限购人数");	
		} 
		$data['is_limit_user'] = 1;//强制默认限购，保证数字权益
