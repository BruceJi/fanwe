...
//				$refund_value['ex_contact'] = '"' . iconv('utf-8','gbk',$list[$k]['ex_contact']) . '"';
	...
	public function set_refund_params(){
		$sys = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."sys limit 0,1");
		if(!empty($sys)){
			$this->assign("withdraw_fee",$sys["withdraw_fee"]);
		}
		$this->display();
	}

	public function format_match($inStr){
		if(empty($inStr))
			return false;

		$pos = strpos($inStr,":");
		$len = strlen($inStr);
		$pre = substr($inStr,0,$pos);
		$post = substr($inStr,$pos+1,$len-$pos-1);  
		$pre_regex = "/^[0-9]+$/";
		$post_regex = "/^0.[0-9]+$/";
		if(!preg_match($pre_regex,$pre)){
			return false;
		}
		if(!preg_match($post_regex,$post)){
			return false;
		}

		$pre_int = intval($pre);
		$post_float = floatval($post); 

		return true;
	}

	public function update_refund_params(){
		$withdraw_fee = trim($_REQUEST["withdraw_fee"]);   
		if(empty($withdraw_fee)){
			$this->error("提现手续费设置不能为空");
		}
		if(!$this->format_match($withdraw_fee)){
			$this->error("提现手续费设置不正确");
		}

		$sys = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."sys limit 0,1");
		if(empty($sys)){ 
			$sys["withdraw_fee"] = $withdraw_fee;
			$GLOBALS['db']->autoExecute(DB_PREFIX."sys",$sys,"INSERT","","SILENT");
		}else{  
			$sys["withdraw_fee"] = $withdraw_fee;
			//echo("update ".DB_PREFIX."sys set withdraw_fee = '".$withdraw_fee."' where id = ".$sys['id']);
			$GLOBALS['db']->query("update ".DB_PREFIX."sys set withdraw_fee = '".$withdraw_fee."' where id = ".$sys['id']);
			//$GLOBALS['db']->autoExecute(DB_PREFIX."sys",$sys,"UPDATE","","SILENT");
		}
